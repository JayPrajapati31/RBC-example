...
int main(int argc, char** argv)
{
   ...
   for(int i=0;i<101;  ++i)
    {
      ...
      double y = 10*x*(x-0.1)*(x-0.25) +0.5;
      xy.push_back(x);
      xy.push_back(y);
      }
  ...
  // Solve parameters
  rbc::SolveParams solve_params({.EPS = 1.e-8, .resScale = 1., .dofScale = 1., .nMaxIters = 50, .verbose = false});
  ...
  lbounds[0] = -HUGE_VAL;//10.2;
  lbounds[1] = -HUGE_VAL;//10.2;
  lbounds[2] = -HUGE_VAL;//1.*(M_PI/blade_length);
  lbounds[3] = -(M_PI/180.)*90.;
  lbounds[4] =  y_lBound-0.1;
  ip_lbounds[4] = lbounds[4];
  
  ubounds[0] = HUGE_VAL;//10.2;
  ubounds[1] = HUGE_VAL;//10.2;
  ubounds[2] = HUGE_VAL;//1.0*(M_PI/blade_length);
  ...
  for(int profile=0; profile<m; ++profile)
	...
	in_wire_params.xy[0] -= 0.01;
      ...
      /* manager.AdvanceCut(optim_vars, in_wire_params, lbounds, ubounds, in_wire_config, intermediate_curve,
			 solve_params, nlopt_params, sdTOL, dtTOL, advance_vec, nlopt_wire_params, nlopt_wire_config, 0., 0.);
      ip_ubounds[0] = nlopt_wire_params.H + 25.0;
      ip_ubounds[1] = nlopt_wire_params.V + 25.0;
      ip_ubounds[2] = nlopt_wire_params.M + 0.25*(M_PI/blade_length);
      ip_ubounds[3] = nlopt_wire_params.BC + 30.*(M_PI/180.);
      ip_lbounds[0] = nlopt_wire_params.H - 25.0;
      ip_lbounds[1] = nlopt_wire_params.V - 25.0;
      ip_lbounds[2] = nlopt_wire_params.M - 0.25*(M_PI/blade_length);
      ip_lbounds[3] = nlopt_wire_params.BC - 30.*(M_PI/180.);
      manager.AdvanceCut(optim_vars, nlopt_wire_params, ip_lbounds, ip_ubounds, nlopt_wire_config, intermediate_curve,
      solve_params, ipopt_params, sdTOL, dtTOL, advance_vec, out_wire_params, out_wire_config, 0., 0.);*/
  ...
  for(int nIter =0; nIter<nRoughCuts_longBlade; ++nIter)
      ...
      for(int k=0; k<(int)(uncut.size()/2); ++k)
	{
	  ...
	  //weightFuncFixNode(temp_wire_config, uncutRange,init_curve, temp_wire_params.node , t_fix);
	  ...
	  manager.AdvanceCut(optim_vars, temp_wire_params, lbounds, ubounds, temp_wire_config, target_curve,wfunc,			     
			     solve_params, nlopt_params, sdTOL, dtTOL, advance_vec, out_wire_params, out_wire_config, 0.0, 0.0);
	  /*manager.AdvanceCut(optim_vars, temp_wire_params, lbounds, ubounds, temp_wire_config, target_curve,wfunc,			     
			     solve_params, nlopt_params, sdTOL, dtTOL, advance_vec, nlopt_wire_params, nlopt_wire_config, 0.0, 0.0);
	  
	  std::cout << "\n nlopt fix node :" << nlopt_wire_params.node << " "
		    << "nlopt origin  (" << nlopt_wire_params.xy[0] << "," << nlopt_wire_params.xy[1] << ")" << std::endl;
	  ...
	  manager.AdvanceCut(optim_vars, nlopt_wire_params, ip_lbounds, ip_ubounds, nlopt_wire_config, target_curve,wfunc,			     
  ...
  /*  std::vector<double> uncut = uncutVec(manager.GetCutProfileMonitor());
  double uncutRange[2] = {uncut[0] , uncut[1]};
  int node;
  double tparam;
  weightFuncFixNode(temp_wire_config, uncutRange,init_curve, node, tparam);*/
  //xy_map_alphaR(endData, blade_length, redundentLen);
}// main function ends
...
/*void weightFuncFixNode(const rbc::WireConfig config, const double* weightRange, const rbc::ParametricCurve& init,  int& node, double& tparam)
{
  const int nNodes = config.theta.size();
  const std::vector<double> coords =  config.xy;
  node = 0;

  std::vector<int> nodeArray;
  for(int i=0; i<nNodes; ++i)
    {
      if(coords[2*i] >= weightRange[0] && coords[2*i] <= weightRange[1])
	{
	  nodeArray.push_back(i);
	}
      if(coords[2*i] > weightRange[1])
	break;
    }
  ...
  double t, t0, t1, dt;
  init.GetParameterBounds(t0, t1);
  int nT = 100;
  dt = (t1-t0)/(nT-1);
  double X1[2],X2[2] ,dX[2];

  for(int i=0;i<nT-1;++i)
    {
      init.Evaluate(i*dt, X1, nullptr);
      init.Evaluate((i+1)*dt, X2, nullptr);
      if(coords[2*node] >= X1[0] && coords[2*node] <= X2[0])
	{
	  t = (i+0.5)*dt;
	  std::cout << "\n\n t is " << t << std::endl;
	}
  ...
  tparam = t;
  std:: cout << "node to be fixed" << node << "\t" << tparam << std::endl;
  }*/
